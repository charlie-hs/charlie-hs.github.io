...
#### Step by Step 표준화 계산 예제 (2023-22) ★★★

> *"입력 데이터가 다음과 같이 나이를 나타내는 특정 데이터로 주어졌을 때, 24세에 해당하는 정규화 결과는?"* (단, 정규화 = 정규분포 표준화)
>
> 데이터: **40, 30, 22, 24**

**Step 1 — 평균(μ)** 계산:

> μ = (40 + 30 + 22 + 24) / 4 = **116 / 4 = 29**
...
> 💡 *주의*: 시험은 *모분산* (n으로 나눔) 인지 *표본분산* (n−1로 나눔) 인지 명시하지 않을 때가 있다. 이 문제는 *√(196/4) = 7* 로 모분산 기준 답이 깔끔하게 나오므로 모분산 사용. 만약 *√14* 같은 답이 보기에 있다면 *표본분산 (n−1)* 기준이다.
...
> ⚠️ **함정 보기 분석**:
> - ② −5/14 — *분산 49 대신 14로 나눔* (분산·표준편차 혼동)
> - ③ −5/√14 — *표본분산(n−1)* 함정. 196/3 ≈ 65.3 → √14 가 분모로 등장
> - ④ −2/7 — *(24−26)/7* 같은 평균 오류
>
> *분산과 표준편차를 혼동하지 말 것*. 분모는 반드시 **표준편차 σ = √분산**.
