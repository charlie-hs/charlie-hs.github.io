...
### 4.3.1 시험 함정 (2025-19 ③) — TF 와 DF 정의 뒤집기 ★★

> *"TF-IDF는 단어마다 가중치를 부여하여 단어를 벡터로 변환하는 방법으로 TF는 특정 단어가 등장한 문서의 수를 의미한다"* → **거짓** ✗

전반부(*단어를 가중치로 벡터화*)는 맞지만, **TF 정의가 뒤집혔다**. *문서의 수* 는 *DF(Document Frequency)* 다.

| 기호 | 정확한 정의 | 시험 함정 형태 |
|---|---|---|
| **TF (Term Frequency)** | *문서 한 편 안에서 단어가 등장한 횟수* | "단어가 등장한 문서의 수" → **거짓** |
| **DF (Document Frequency)** | *전체 문서 중 그 단어가 한 번이라도 등장한 문서의 수* | TF 정의로 뒤집어 출제 |
| **IDF (Inverse DF)** | *log(N / DF)* — 흔할수록 작아짐 | — |

> 💡 **암기 한 줄**: *"TF 는 한 문서 안의 빈도, DF 는 그 단어를 가진 문서의 개수."* — TF 의 *F = Frequency in one doc*, DF 의 *F = Frequency across docs*. 둘 다 *Frequency* 라 헷갈리게 출제됨.

> ⚠️ 같은 문항 *2025-19* 의 다른 보기들은 모두 참:
> - ① *"N-gram 은 문자열에서 N개의 연속된 요소"* — 참
> - ② *"원-핫 인코딩은 단어 개수가 많아질수록 벡터 저장 공간이 늘어나는 단점"* — 참 (*어휘 폭발 문제*, §3.3 참조)
> - ④ *"Word2Vec 은 비슷한 컨텍스트의 단어들이 유사한 의미"* — 참 (*분포 가설*, §6 참조)
> 그래서 ③ 이 정답.
