...
### 5.6 시험 함정 (25-11 ③) — 데이터 증강 vs 오토인코더 ★

> *"데이터 증강은 입력 데이터를 저차원 데이터로 압축하는 인코더 형태의 모델을 통해 학습한다"* → **거짓** ✗

이건 *오토인코더(Autoencoder)* 의 설명이다. 데이터 증강과 오토인코더는 *완전히 다른 개념* — 한 줄에서 헷갈리게 만든 함정.

| 개념 | 정의 | 목적 |
|---|---|---|
| **데이터 증강** | *원본 데이터에 변형* (회전·반전·노이즈 등) 을 가해 *새 학습 데이터를 추가* | *데이터 양 ↑* → 과적합 완화 |
| **오토인코더** | *입력을 저차원 잠재 표현 z 로 압축한 뒤 다시 복원* 하는 인코더-디코더 신경망 | *차원 축소·표현 학습·이상 탐지* |

```mermaid
flowchart LR
    subgraph DA["데이터 증강 (Data Augmentation)"]
        direction LR
        I1["원본 이미지"] --> T1["회전·반전·노이즈"]
        T1 --> O1["변형된 새 학습 샘플"]
    end

    subgraph AE["오토인코더 (Autoencoder)"]
        direction LR
        I2["입력 x"] --> Enc["인코더"]
        Enc --> Z["저차원 z<br/>(병목층)"]
        Z --> Dec["디코더"]
    ...
    style DA fill:#d4f4dd
    style AE fill:#ffe8cc
```

> 🎯 **암기 한 줄**: *"데이터 증강은 데이터를 *늘리고*, 오토인코더는 데이터를 *압축한다*."* — *늘림 ↔ 압축* 으로 정반대 방향.

> ⚠️ 같은 *2025-11* 문항의 ④ *"프로그램을 명시적으로 작성하지 않고 컴퓨터가 학습할 수 있는 능력을 갖게 하는 기술이다"* → **참** (Samuel 1959 의 *기계학습* 정의). ①②④ 가 모두 참이라 *③ 이 정답*.
