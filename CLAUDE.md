...
> 본 위키의 글은 **본인 학습용**일 뿐 아니라 **불특정 다수의 학습자**가 참고하는 자료. 따라서 모든 사실 기반 정보는 **공신력 있는 출처로 교차 검증**해야 한다.

### 자료 신뢰도 우선순위 (높음 → 낮음)

| 순위 | 자료 유형 | 예시 |
|---|---|---|
| 1 | **학술 논문** (peer-reviewed, arXiv 포함) | Turing(1950), Rumelhart et al.(1986), Vaswani et al.(2017) |
| 2 | **출간된 교과서** | Russell & Norvig (AIMA), Goodfellow *Deep Learning*, Bishop *PRML* |
| 3 | **공식 문서·표준** | IEEE, ISO, 정부 공식 자료(과기정통부 등), 기관 공식 페이지(OpenAI/DeepMind/Google AI 공식 블로그) |
| 4 | **참고용 PDF**(강의 노트 등) | 사용자 제공 자료 — 시작점일 뿐, **토대가 아님** |
| 5 | **개인 블로그·매체 기사** | 위 1~4에 정보가 없을 때만 보완용 |

### 작성 규칙 (Critical)

- **사용자가 제공한 PDF·자료는 시작점일 뿐 토대가 아님.** 반드시 1~3순위 자료로 교차 검증할 것
- **연도·인물·논문 제목·핵심 정의** 같은 사실 기반 정보는 원전(논문 또는 공식 출처) 확인 필수
- **인용 시 출처 명시**: 본문 또는 부록에 명확한 레퍼런스 추가
  - 논문: 저자, 제목, 학회/저널, 연도, DOI 또는 arXiv ID
  - 책: 저자, 제목, 출판사, 연도, (가능하면) 페이지
  - 웹: 제목, 사이트, URL, 접근일
- 정보가 1~3순위에서 확인되지 않으면 그 사실을 본문에 명시 ("출처 미확인" 또는 "추가 검증 필요")
- 통념·도시전설(예: "Minsky의 *Perceptrons*가 1차 AI 겨울을 직접 야기")은 학술적 합의 여부를 확인 후 신중히 표현

### 시각 자료 표준

- **다이어그램은 가능한 많이 사용**. 이해를 돕는 데 핵심
- 우선순위:
  1. **Mermaid** 코드블록 — 텍스트 기반, 버전관리 가능, Notion·GitHub 양쪽 지원
  2. **Markdown 표** — 비교·매트릭스에 적합
  3. **ASCII 다이어그램** — 간단한 포함관계·흐름
  4. **이미지** — 위 1~3으로 표현 어려운 경우만 (직관적 사진, 복잡한 아키텍처 등)
- 이미지 사용 시: 본인이 직접 만들 수 없으므로 **이미지 생성용 프롬프트를 본문에 함께 제공**
  - 형식: ```![대안 텍스트](경로) <!-- prompt: "..." -->```
  - 또는 별도 부록 섹션에 프롬프트 모음 정리

### 글의 톤·형식

- 시험 대비 자료라도 **시험만을 위한 글이 아님**. 일반 학습자에게도 이해 가능한 자기-완결형 글로 작성
- 정의 → 직관 → 핵심 메커니즘 → 응용 → 한계의 순서
- **헷갈리는 것**·**자주 묻는 질문** 섹션을 적극 활용해 통념 바로잡기
- 마지막에 **참고 문헌(References)** 섹션 필수
